...
export async function POST(req) {
	...
	try {
		...
		const emailRes = await sendEmail({email:user.email , emailType:'verify', userId:user._id});
		return NextResponse.json({ success: true, msg: "Please Check Your Email", user , emailRes }, { status: 201 });
	} catch (error) {
